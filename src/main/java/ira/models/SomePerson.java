...
public abstract class SomePerson {
    ...
    public void attack(SomePerson target) {
        ...
        if (target.getSomeStatus() == EnumSomeStatus.IN_DEFENCES) {
            ...
            tempPowerInDef*=xDef;
            target.setHealth(target.getHealth()-tempPowerInDef);
            System.out.printf(
                    "%s защитился! %s нанес всего %d урона!\nУ %s осталось %d здоровья!\n",
                    target.getName(), this.getName(), (int) tempPowerInDef, target.getName(), (int) target.getHealth());
            ...
            target.setHealth(target.getHealth() - tempPower);
            System.out.printf(
                    "%s нанес %d урона!\nУ %s осталось %d здоровья!\n",
                    this.getName(), (int) tempPower, target.getName(), (int) target.getHealth());
            if (target.getHealth() <= 0) {
                target.setAlive(false);
                System.out.printf("%s повержен...\n", target.getName());
            }
